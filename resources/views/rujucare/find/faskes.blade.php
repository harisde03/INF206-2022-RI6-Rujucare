...
@section('content')
    ...
    <section class="container-xl min-vh-90">
        <div class="row justify-content-center">
            <div class="col-md-12">
                <form class="nosubmit position-relative" name="searchFaskes" action="{{ route('cari.faskes') }}">
                    @csrf
                    <div class="">
                        <input class="form-control nosubmit position-absolute top-50 start-50 translate-middle"
                            autocomplete="off" type="search" name="searchFaskes" placeholder="Temukan Fasilitas Kesehatan"
                            value="">
        ...
        @if ($post->count())
            <table class="mb-3">
                <tr>
                    ...
                    <th style="width:34.5%">Tingkat Fasilitas Kesehatan</th>
                    <th style="width:27.75%">Kamar Tersedia</th>
                </tr>

                @foreach ($post as $p)
                    <tr>
                        <td> <a href="/faskes/{{ $p->urlFaskes }}">{{ $p->namaPublik }}</a></td>
                        <td>{{ $p->tingkatPublik }}</td>
                        <td>{{ $p->ketersediaan->kamarTersedia }}</td>
                    </tr>
                @endforeach
